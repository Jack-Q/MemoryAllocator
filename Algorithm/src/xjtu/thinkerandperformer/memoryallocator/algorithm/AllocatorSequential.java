    ...
    public void show() {
        for (int i = 0; i < memPool.length; i++) {
            if (memPool[i] == FREE || memPool[i] == RESERVED) {
                if (memPool[i] == FREE) {
                    int size = memPool[i + FULL_SIZE];
                    int end = i + size + FREE_END_TAG;

                    System.out.printf("%3d %s %3d %s", i, "-", end, ":空闲 [");

                    System.out.printf("%8s", "size:" + memPool[i + FULL_SIZE]);
                    System.out.print(" ");
                    System.out.printf("%11s", "end_size:" + memPool[end - 1]);

                    System.out.printf("%8s", "LPTR:" + memPool[i + L_PTR]);
                    System.out.printf("%8s", "RPTR:" + memPool[i + R_PTR]);
                    ...
                    int size = memPool[i + FULL_SIZE];
                    int end = i + size + RES_END_TAG;

                    System.out.printf("%3d %s %3d %s", i, "-", end, ":占用 [");

                    System.out.printf("%8s", "size:" + memPool[i + FULL_SIZE]);
                    System.out.print(" ");
                    System.out.printf("%11s", "user_size:" + memPool[i + USER_SIZE]);
